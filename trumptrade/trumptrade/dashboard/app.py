...
tab_pos, tab_alerts, tab_markets, tab_sources, tab_risk = st.tabs(
    ["positions", "alerts", "markets", "sources", "risk"]
)
...
with tab_pos:
    ...
    if not open_df.empty:
        ...
        col3.metric("open notional ($)", f"{notional:,.2f}")
        if "current_mark" in open_df.columns:
            ...
            col4.metric("unrealized P&L ($)", f"{unreal:+,.2f}")
    if not closed_df.empty and "realized_pnl" in closed_df.columns:
        st.caption(f"realized P&L all-time: ${closed_df['realized_pnl'].sum():+,.2f}")
